...
def genererate_data_user(count_user, count_bookmarks, count_reviews, count_likes_reviews, size_header, size_text):
    res=[]
    for i in range(count_user):
        ...
        for i in range(count_reviews):
            ...
            for i in range(count_likes_reviews):
                reviews["likes"].append({"user_id": bson.Binary.from_uuid(uuid.uuid4())})
...
def create_data_for_insert_mongo(count_filmwork,count_scores, count_reviews, count_like_reviews, size_header, size_text,
                      count_user, count_bookmarks):

    data_filmworks = genererate_data_filmwork(count_filmwork,count_scores, count_reviews, count_like_reviews, size_header, size_text)
    data_users = genererate_data_filmwork(count_user, count_bookmarks, count_reviews, count_like_reviews, size_header, size_text)
...
def find_expected_value_for_id_filmwork(db, id):
    pipeline = [
        {'$match': {'id': id}},  # Match all documents
        {'$unwind': '$scores'},
        {'$group': {
            '_id': 'null',
