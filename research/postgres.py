...
def generate_table_reviews_user(count):
   ...
   # return [{'user_id':uuid.uuid4(), 'filmwork_id':uuid.uuid4(),'header':random_str(size_header),
   #          'text':random_str(size_text), 'created_at':datetime.datetime.now()} for i in range(count)]
