...
class FilmworkService:
    ...
    async def upsert_user_score(
        self, filmwork_id: UUID, user_id: UUID, score: int
    ) -> None:
    ...
    async def _filmwork_already_scored_by_user(
        self, filmwork_id: UUID, user_id: UUID
    ) -> bool:
        return (
            await Filmwork.find(
    ...
    async def _insert_filmwork_score(
        self, filmwork_id: UUID, user_id: UUID, score: int
    ) -> None:
    ...
    async def _update_filmwork_score(
        self, filmwork_id: UUID, user_id: UUID, score: int
    ) -> None:
